...
local nlua = vim.api.nvim_get_runtime_file("lua/nlua", false)[1]
...
function BustedNeotestAdapter.build_spec(args)
  ...
  local command = { "busted", "--lua", nlua, "-m", "'lua/?.lua;lua/?/init.lua'" }
...
---@param result BustedTestResult
local function convert_plenary_result(result, status, file)
  local t = table.concat(vim.iter { file, result.descriptions, "::" }:flatten():totable())
  return t, {
    status = status,
    short = result.msg,
...
local function permutations(lists, cur_i)
  ...
  for _, elem in pairs(lists[cur_i]) do
    for _, sub_result in pairs(sub_results) do
      ...
      table.insert(reulst, l)
    end
  end
  return reulst
end

---@class BustedTestResult
---@field descriptions string[]
---@field msg? string

---@class BustedTestResults
---@field pass BustedTestResult[]
---@field fail BustedTestResult[]
---@field errs BustedTestResult[]
---@field fatal BustedTestResult[]

---@class BustedOutput
---@field results BustedTestResults
...
function BustedNeotestAdapter.results(spec, _, tree)
  if tree:data().type == "file" and #tree:children() == 0 then
    tree = BustedNeotestAdapter.discover_positions(tree:data().path)
  end
...
BustedNeotestAdapter.setup = function(opts)
  opts = opts or {}
